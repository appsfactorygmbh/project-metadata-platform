...
<template>
  <a-form ref="formRef" :model="dynamicValidateForm">
    <a-form-item :name="props.attributeName" :rules="currentAttributeRulesForTemplate" class="formItem"
      :has-feedback="!!(props.rules && props.rules.length > 0)">
      <a-select v-model:value="dynamicValidateForm[props.attributeName]" show-search :placeholder="props.placeholder"
        :options="props.options" :field-names="{ label: 'name', value: 'id' }" :dropdown-match-select-width="false"
