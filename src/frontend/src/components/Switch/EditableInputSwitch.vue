    ...
    <template v-if="!isLoading">
      <a-form ref="formRef" :model="dynamicValidateForm" class="switch-form">
        <a-form-item :name="props.attributeName" class="formItem">
          <a-switch v-model:checked="dynamicValidateForm[props.attributeName]" class="custom-color-switch"
            :disabled="!isEditing" />
        </a-form-item>
      </a-form>

      <EditButtons v-if="hasEditKeys" class="editButton" :is-editing-key="props.isEditingKey"
        :is-loading="props.isLoading" :safe-disabled="props.isLoading" :form-store="props.formStore"
        @saved-changes="emit('savedChanges')" />
    </template>

    <a-skeleton v-else active :paragraph="false" style="margin-left: 1em; width: 4em" />
  </a-card>
</template>
