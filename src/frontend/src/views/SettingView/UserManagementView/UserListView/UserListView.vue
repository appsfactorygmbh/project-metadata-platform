...
onMounted(async () => {
  ...
  if (routerUserId.value) { await userStore?.fetchUser(routerUserId.value); }
  selectedKeys.value = [routerUserId.value];
});
</script>
  ...
  <a-layout class="layout">
    <a-layout-sider v-model:collapsed="collapsed" class="sideSlider" collapsible :width="250">
      <a-menu v-if="!isLoading" v-model:selected-keys="selectedKeys" mode="inline" class="menuItem">
        <a-menu-item key="create-user" class="create-menu-item"
        ...
        <a-menu-item v-for="user in usersData" :key="user.externalId" :title="getNameFromEmail(user.userName)"
