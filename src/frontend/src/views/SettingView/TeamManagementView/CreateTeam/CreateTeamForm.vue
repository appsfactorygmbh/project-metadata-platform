...
<template>
  <a-form ref="formRef" :model="dynamicValidateForm" v-bind="formItemLayoutWithOutLabel" :wrapper-col="{ span: 24 }">
    <a-form-item has-feedback name="teamName" class="column" :whitespace="true" :rules="rulesRef.teamName">
      <a-input id="inputCreateTeamTeamName" v-model:value="dynamicValidateForm.teamName" class="inputField"
        placeholder="Team Name" :disabled="dynamicValidateForm.inputsDisabled" :rules="rulesRef.teamName" />
    </a-form-item>
    <a-form-item has-feedback name="businessUnit" class="column" :whitespace="false" :rules="rulesRef.businessUnit">
      <a-select id="inputCreateTeamBusinessUnit" v-model:value="dynamicValidateForm.businessUnit"
        placeholder="Business Unit" :disabled="dynamicValidateForm.inputsDisabled">
        <a-select-option v-for="bu in getBusinessUnits" :key="bu.id" :value="bu.id">{{ bu.businessUnitName
          }}</a-select-option>
      </a-select>
    </a-form-item>
    <a-form-item has-feedback name="ptl" class="column" :whitespace="true" :rules="rulesRef.ptl">
      <a-input id="inputCreateTeamPtl" v-model:value="dynamicValidateForm.ptl" class="inputField" placeholder="PTL"
        :disabled="dynamicValidateForm.inputsDisabled" :rules="rulesRef.ptl" />
    </a-form-item>
  </a-form>
  <contextHolder />
</template>
