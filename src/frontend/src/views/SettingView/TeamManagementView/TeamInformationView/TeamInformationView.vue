...
<template>
  <ConfirmationDialog :is-open="isConfirmModalOpen" title="Delete confirm"
    message="Are you sure you want to delete this team?" @confirm="deleteTeam" @cancel="closeModal"
    @update:is-open="isConfirmModalOpen = $event" />
  <div v-if="team && team.id" class="panel">
    <a-flex class="userInfoBox" :body-style="{
      height: 'fit-content',
    }">
      <EditableTextField class="textField teamName" :value="team?.teamName ?? ''" :is-loading="isLoading"
        :label="'Team\xa0Name'" :is-editing-key="'isEditingTeamName'" :form-store="teamNameFormStore"
        :has-edit-keys="true">
        <TeamNameInputField :team-id="team?.id ?? -1" :form-store="teamNameFormStore"
          :placeholder="team?.teamName ?? ''" :default="team?.teamName ?? ''" />
      </EditableTextField>

      <EditableTextField :value="team == undefined ? '' : (team.businessUnit.businessUnitName ?? '')"
        :label="'Business\xa0Unit'" :is-editing-key="'isEditingBU'" :is-loading="isLoading"
        :form-store="businessUnitFormStore" :has-edit-keys="true">
        <TeamInformationSearchSelectField :team-id="team?.id ?? 0" :attributeName="'businessUnitId'"
          :form-store="businessUnitFormStore" :default="team?.businessUnit?.businessUnitName ?? ''"
          :placeholder="team?.businessUnit?.businessUnitName ?? ''" :options="buStore.getBusinessUnits.map((bu) => ({
            id: bu.id,
            name: bu.businessUnitName,
          }))" />
      </EditableTextField>
      <EditableTextField :value="team == undefined ? '' : (team.ptl ?? '')" :label="'PTL'"
        :is-editing-key="'isEditingPTL'" :is-loading="isLoading" :form-store="ptlFormStore" :has-edit-keys="true">
        <TeamInformationInputField :team-id="team?.id ?? -1" :attributeName="'ptl'" :form-store="ptlFormStore"
          :default="team == undefined ? '' : (team.ptl ?? '')" :placeholder="'PTL'">
        </TeamInformationInputField>
      </EditableTextField>
    </a-flex>
  </div>
  <a-skeleton v-else-if="isLoading" :description="`No Team Found for Id ${route.query.teamId}`"></a-skeleton>
  <a-empty v-else-if="route.query.teamId" :description="`No Team Found for Id ${route.query.teamId}`"></a-empty>
