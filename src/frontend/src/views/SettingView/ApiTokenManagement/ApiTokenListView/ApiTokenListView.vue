...
onMounted(async () => {
  ...
  if (routerApiTokenId.value) { await apiTokenStore?.fetchApiToken(Number(routerApiTokenId.value)); }
  selectedKeys.value = [routerApiTokenId.value];
});
</script>
...
<template>
  <a-layout class="layout">
    <a-layout-sider v-model:collapsed="collapsed" class="sideSlider" collapsible :width="250">
      <a-menu v-if="!isLoading" v-model:selected-keys="selectedKeys" mode="inline" class="menuItem">
        <a-menu-item key="create-token" class="create-menu-item"
        ...
        <a-menu-item v-for="ApiToken in tokensData" :key="ApiToken.id" :title="ApiToken.name"
      ...
      <a-skeleton v-else active :paragraph="false" style="margin-left: 1em; width: 15em" />
    </a-layout-sider>
    <a-layout-content>
      <div class="content">
        <RouterView />
