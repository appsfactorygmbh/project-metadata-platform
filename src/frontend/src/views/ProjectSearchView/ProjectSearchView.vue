...
const toSearchModelConverter = (model: ProjectModel) => {
  return {
    ...
    businessUnit: model.team === undefined ? '' : model.team.businessUnit.businessUnitName,
  };
};
  ...
  <div style="padding: 10px">
    <a-flex vertical gap="middle">
      <span style="display: flex; flex-direction: row">
        ...
        <a-tooltip placement="left" title="Click here to reset all filters" style="padding-left: 0; padding-right: 0">
          <a-button class="button" name="resetButton" :style="highlightButtonStyle" @click="clearAllFilters">
            <template #icon>
              <UndoOutlined class="icons" />
            </template>
          </a-button>
        </a-tooltip>
        <a-tooltip placement="left" title="Click here to toggle between active and archived projects">
      ...
      <SearchableTable :search-store-symbol="searchStoreSymbol" :pane-height="props.paneHeight"
        :columns="[...columns.filter((item) => !item.hidden)]" :is-loading="isLoading!" @row-click="handleRowClick" />
    </a-flex>
  </div>
</template>
